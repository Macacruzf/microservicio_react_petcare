...
public class CarritoController {
    ...
    @Operation(
            ...
            description = "Agrega un producto al carrito. Si el carrito no existe, se crea automáticamente."
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
    ...
    public ResponseEntity<CarritoDto> agregar(
            ...
            @RequestBody
            @Schema(description = "Datos del producto que se agregará al carrito",
                    implementation = CarritoItemDto.class)
            CarritoItemDto itemDto
    ) {
        return ResponseEntity.ok(carritoService.agregarProducto(usuarioId, itemDto));
    }
